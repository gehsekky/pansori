...
interface Theme {
  pageBg: string; cardBg: string; font: string;
  primary: string; mid: string; dim: string; dimDark: string;
  border: string; separator: string; itemColor: string;
  hpHigh: string; hpMid: string; hpLow: string;
  title: string; worldLabel: string;
}
...
export default function App() {
  ...
  const {
    session, gameState, seed, choices, history,
    loading, escaped, roomLog,
          ...
          <div className={styles.page}>
                ...
                <div className={styles.headerLeft}>
                  ...
                  <div>
                    ...
                    <p className={styles.sub}>
                      {ctx.theme.worldLabel}: {worldName} · ACTIVE: {activeChar?.name} [{activeChar?.character_class}]
                    </p>
                  </div>
                </div>
                {user && (
            ...
            <div className={styles.contentRow}>
              <div className={styles.contentMain}>
                <div className={styles.card} style={{ maxHeight: 320, overflowY: 'auto' }} ref={narrativeRef}>
                  ...
                  {loading && roomLog.length > 0 && (
                    <p className={styles.scanTxt} style={{ marginTop: '0.5rem' }}>Scanning sector...</p>
                  )}
                </div>

                {!loading && escaped ? (
                  <div className={styles.card} style={{ borderColor: 'var(--t-primary)', textAlign: 'center', padding: '1.5rem' }}>
                    ...
                    <button className={styles.submit} style={{ width: 'auto', padding: '0.6rem 2rem' }} onClick={startNewMission}>
                      START NEW MISSION
                    </button>
                  </div>
                ) : !loading && allDead ? (
                  <div className={styles.card} style={{ borderColor: 'var(--t-hp-low)', textAlign: 'center', padding: '1.5rem' }}>
                  ...
                  <div className={styles.choices}>
                    {!loading && choices.map((c, i) => (
                      <button key={i} className={styles.choiceBtn} onClick={() => handleChoice(c)}>
                        [{i + 1}] {c.label}
                      </button>
                    ))}
                  </div>
                ...
                {history.length > 0 && (
                  <div className={styles.card} style={{ marginTop: '1.5rem', maxHeight: 160, overflowY: 'auto' }} ref={logRef}>
                    ...
                    {[...history]
                      ...
                      .slice(0, 20)
                      .map((m, i) => (
                        <p key={i} className={styles.logEntry}>› {m.content}</p>
                      ))}
                  </div>
                )}
