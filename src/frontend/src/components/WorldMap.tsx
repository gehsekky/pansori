    ...
    <div className={styles.mapOverlay} onClick={onClose}>
      <div className={styles.mapBox} onClick={e => e.stopPropagation()}>
          ...
          {edges
            ...
            .map(({ a, b, cross }) => {
              ...
              if (cross) {
                ...
                return (
                  <path key={`e${a}-${b}`} d={`M ${x1} ${nodeY} Q ${mx} ${arc} ${x2} ${nodeY}`}
                    fill="none" stroke="var(--t-border)" strokeWidth={1.5} strokeDasharray="4 3" />
                );
              }
              return (
                <line key={`e${a}-${b}`} x1={x1} y1={nodeY} x2={x2} y2={nodeY}
                  stroke="var(--t-border)" strokeWidth={1.5} />
              );
            })}

          {rooms.filter(room => revealed(room.id)).map(room => {
            ...
            return (
              <g key={room.id}>
                {isCur && (
                  <circle cx={x} cy={nodeY} r={R + 6} fill="none"
                    stroke="var(--t-primary)" strokeWidth={1} opacity={0.35}
                    style={{ filter: 'blur(3px)' }} />
                )}
                <circle cx={x} cy={nodeY} r={R} fill={fill} stroke={stroke} strokeWidth={isCur ? 2 : 1.5} />
                {hasEnemy && (
                  <circle cx={x + R - 4} cy={nodeY - R + 4} r={5}
                    fill="var(--t-hp-low)" stroke="var(--t-card)" strokeWidth={1.5} />
                )}
                {hasLoot && (
                  <circle cx={x - R + 4} cy={nodeY - R + 4} r={5}
                    fill="var(--t-hp-high)" stroke="var(--t-card)" strokeWidth={1.5} />
                )}
                <text x={x} y={nodeY + R + 14} textAnchor="middle" fontSize={9}
                  fill={tColor} fontFamily="var(--t-font)" letterSpacing="0.06em">
                  {label}
                </text>
              </g>
            );
        ...
        <div className={styles.mapLegend}>
          <span><span style={{ color: 'var(--t-primary)' }}>●</span> YOU ARE HERE</span>
          <span><span style={{ color: 'var(--t-mid)' }}>●</span> VISITED</span>
          <span><span style={{ color: 'var(--t-border)' }}>●</span> UNKNOWN</span>
          <span><span style={{ color: 'var(--t-hp-low)' }}>●</span> ENEMY</span>
          <span><span style={{ color: 'var(--t-hp-high)' }}>●</span> LOOT</span>
        </div>
      </div>
    </div>
  );
