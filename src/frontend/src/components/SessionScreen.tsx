    ...
    <div className={styles.pageFlex}>
      <div className={styles.sessionsInner}>
        <div className={styles.sessionsHeader}>
          <div>
            <p className={styles.title} style={{ fontSize: '1.1rem', marginBottom: 4 }}>PANSORI</p>
          ...
          <div className={styles.sessionsActions}>
            ...
            <button className={styles.ghostBtn} onClick={onLogout}>SIGN OUT</button>
          </div>
        </div>

        {sessions.length === 0 ? (
          <div className={styles.card} style={{ textAlign: 'center', padding: '2.5rem', color: 'var(--t-dim)' }}>
            ...
            {sessions.map(s => {
              ...
              return (
                <div
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: 0, opacity: isActive ? 1 : 0.6 }}
                >
                  {s.portrait_url
                  ...
                  {isActive && (
                    <button className={styles.sendBtn} style={{ flexShrink: 0 }} onClick={() => onResume(s.id)} disabled={loading}>
                      RESUME
                    </button>
                  )}
                  <button
                    className={styles.ghostBtn}
                    style={{ padding: '0.3rem 0.6rem', fontSize: '0.75rem', letterSpacing: '0.08em', flexShrink: 0 }}
                    onClick={() => {
                      const msg = isActive
