...
export const SRD_SPELLS: Record<string, Spell> = {
  ...
  spare_the_dying: {
    ...
    stabilizes: true,
    narratives: {
      cast: ['{name} steadies {target}, pulling them back from the brink — they stabilize.'],
    },
    desc: 'Stabilize a dying creature within 15 ft (range increases with level). It stops making death saves.',
    rangeKind: 'ranged',
    rangeFt: 15,
  ...
  remove_curse: {
    ...
    desc: 'Touch: end all curses on the target (the Bestow Curse debuff ends) and break the attunement bond on any cursed items they carry — the item stays cursed, but it can finally be put down.',
    rangeKind: 'touch',
    spellList: ['arcane', 'divine'],
  },
  ...
  silence: {
    ...
    ritualCasting: true,
    concentration: true,
    durationRounds: 100, // up to 10 minutes
    rangeKind: 'ranged',
    rangeFt: 120,
    blastRadius: 20,
    desc: 'A 20-ft-radius sphere of utter quiet: a creature inside can’t cast a spell with a Verbal component (Concentration, up to 10 minutes).',
  ...
  fog_cloud: {
    ...
    rangeFt: 120,
    blastRadius: 20,
    desc: 'A 20-ft-radius sphere of fog Heavily Obscures its area — sight (including Darkvision) can’t reach through it, blinding combat within (Concentration, up to 1 hour).',
  ...
  teleport: {
    ...
    desc: 'You and your companions are instantly transported to a destination you know — any town the party has visited. No travel time passes. (The off-target / mishap table for unfamiliar destinations is narrated, not rolled.)',
    narrative:
  ...
  word_of_recall: {
    ...
    rangeKind: 'self',
    somatic: false, // SRD: V only
    desc: 'You and your companions are instantly teleported to the sanctuary you consecrated in advance — cast this spell in a town to designate it; cast it anywhere else to return.',
  ...
  // ─── Out-of-combat utility / exploration spells ──────────────────────────────
  // These SRD spells have no combat-mechanical effect to model — their RAW effect
  // is narrative, exploratory, or DM-adjudicated (planar travel, illusions of
  // place, wards, social influence). Like Augury / Commune / Divination /
  // Legend Lore, they resolve as narrative utility (the faithful representation
  // in a text engine). `outOfCombatOnly` gates the long (1 min+) casts.
  arcanists_magic_aura: {
    ...
    castTime: 'action',
    rangeKind: 'touch',
    desc: 'Place an illusory magical aura (or mask one) on a creature or object you touch: it can read as a chosen school of magic, or as nonmagical, for 24 hours.',
    narrative: '{name} traces a false sigil; the target now reads however {name} wills.',
    spellList: ['arcane'],
  },
  illusory_script: {
    id: 'illusory_script',
    ...
    outOfCombatOnly: true,
    rangeKind: 'touch',
    desc: 'Write on parchment a message only those you choose can read; everyone else sees indecipherable or innocuous text. Lasts 10 days.',
    narrative: '{name} inks a passage that twists into nonsense for unintended eyes.',
    spellList: ['arcane'],
  },
  private_sanctum: {
    id: 'private_sanctum',
    ...
    rangeKind: 'ranged',
    rangeFt: 120,
    desc: 'Ward an area against scrying, teleportation, and prying senses for 24 hours — sound, sight, and divination can be sealed out as you choose.',
    narrative: '{name} seals the space; the world outside goes blind and deaf to it.',
    spellList: ['arcane'],
  },
  secret_chest: {
    id: 'secret_chest',
    ...
    castTime: 'action',
    rangeKind: 'touch',
    desc: 'Hide a chest (and its replica) on the Ethereal Plane; you can recall or send it away with an action while the replica is at hand.',
    narrative: '{name} touches the chest and it slips out of the world, stowed beyond reach.',
    spellList: ['arcane'],
  },
  contact_other_plane: {
    ...
    outOfCombatOnly: true,
    rangeKind: 'self',
    desc: 'Mentally contact a distant entity for cryptic answers to up to five questions — at the risk of psychic strain to your own mind.',
    narrative: '{name} reaches across the planes; something vast and cold answers.',
    spellList: ['arcane'],
  },
  dream: {
    id: 'dream',
    ...
    outOfCombatOnly: true,
    rangeKind: 'self',
    desc: "Shape a known creature's dreams from afar — deliver a message, or haunt them with nightmares that rob their rest.",
    narrative: '{name} sends a dream walking into a sleeper far away.',
    spellList: ['arcane'],
  },
  teleportation_circle: {
    ...
    outOfCombatOnly: true,
    rangeKind: 'self',
    desc: 'Inscribe a circle linked to a permanent teleportation sigil you know; for one round, anyone stepping through is whisked to that destination.',
    narrative: '{name} chalks a glowing circle that opens onto a distant, familiar place.',
    spellList: ['arcane'],
  },
  guards_and_wards: {
    id: 'guards_and_wards',
    ...
    outOfCombatOnly: true,
    rangeKind: 'touch',
    desc: 'Lace a building with protective magic — fog, locked ways, confusion, and minor hazards — guarding it for 24 hours.',
    narrative: '{name} weaves the halls full of wards, snares, and misdirection.',
    spellList: ['arcane'],
  },
  instant_summons: {
    id: 'instant_summons',
    ...
    outOfCombatOnly: true,
    rangeKind: 'touch',
    desc: 'Mark an object with a paired sapphire; later, crush the sapphire to call the item instantly to your hand from anywhere on the same plane.',
    narrative: '{name} crushes a sapphire and the marked item leaps into their grasp.',
    spellList: ['arcane'],
  },
  transport_via_plants: {
    ...
    castTime: 'action',
    rangeKind: 'touch',
    desc: 'Open a one-round portal between two large plants you know, on the same plane — step through one and out the other.',
    narrative: '{name} parts the boughs and the party steps out a world away.',
    spellList: ['primal'],
  },
  magnificent_mansion: {
    ...
    rangeKind: 'ranged',
    rangeFt: 300,
    desc: 'Conjure an extradimensional dwelling with a fine interior and unseen servants, safe for up to 24 hours — only those you admit may enter.',
    narrative: '{name} opens a shimmering doorway into a grand, welcoming hall.',
    spellList: ['arcane'],
  },
  mirage_arcane: {
    id: 'mirage_arcane',
    ...
    outOfCombatOnly: true,
    rangeKind: 'self',
    desc: 'Reshape the look, sound, and feel of a vast stretch of terrain — a swamp becomes a meadow, a road a chasm — convincing to every sense for up to 10 days.',
    narrative: '{name} rewrites the land; the terrain itself wears a convincing mask.',
    spellList: ['arcane', 'primal'],
  },
  sequester: {
    id: 'sequester',
    ...
    castTime: 'action',
    rangeKind: 'touch',
    desc: 'Hide a creature or object from sight and divination — invisible and undetectable — until a condition you set is met.',
    narrative:
      '{name} touches the target and it slips, unseen and unfindable, out of the world’s notice.',
    spellList: ['arcane'],
  },
  symbol: {
    id: 'symbol',
    ...
    outOfCombatOnly: true,
    rangeKind: 'touch',
    desc: 'Inscribe a harmful glyph on a surface or object; when triggered, it unleashes a chosen effect (fear, pain, sleep, and the like) on those nearby.',
    narrative: '{name} carves a rune that waits, patient and deadly, to be disturbed.',
    spellList: ['arcane', 'divine', 'primal'],
  },
  demiplane: {
    id: 'demiplane',
    ...
    rangeKind: 'ranged',
    rangeFt: 60,
    desc: 'Open a shadowy door to a small empty demiplane — a featureless room you can store creatures or goods in, returning to it later.',
    narrative: '{name} opens a plain door onto a silent, empty room that is nowhere at all.',
    spellList: ['arcane'],
  },
  glibness: {
    id: 'glibness',
    ...
    rangeKind: 'self',
    durationRounds: 600,
    desc: 'For 1 hour, you are flawlessly persuasive — your Charisma checks never roll low, and magic can’t tell whether you speak the truth.',
    narrative: "{name}'s words turn smooth as silk — every one of them lands as truth.",
    spellList: ['arcane'],
  },
  antipathy_sympathy: {
    ...
    rangeKind: 'ranged',
    rangeFt: 60,
    desc: 'Enchant a creature or object so a chosen kind of creature is overwhelmingly repelled (Frightened away) or drawn to it, for 10 days.',
    narrative: '{name} lays a compulsion: some will flee this place, others cannot stay away.',
    spellList: ['arcane', 'primal'],
  },
  clone: {
    id: 'clone',
    ...
    outOfCombatOnly: true,
    rangeKind: 'touch',
    desc: 'Grow an inert duplicate of a creature in a vessel over 120 days; should the original die, its soul transfers to the waiting clone.',
    narrative: '{name} seals flesh and salts into a vessel, where a sleeping double slowly forms.',
    spellList: ['arcane'],
  },
  astral_projection: {
    ...
    outOfCombatOnly: true,
    rangeKind: 'self',
    desc: 'Project your astral body (and willing companions) into the Astral Plane, leaving your physical bodies safe in a trance to travel the planes.',
    narrative:
  ...
  // SRD: True Polymorph — like Polymorph, but at 9th level (and RAW permanent
  // after 1 minute of concentration). Reuses the `polymorphed` transformation
  // (the target becomes a beast for the duration). The any-form / permanent
  // aspects are simplified to the beast-form model + concentration timer.
  true_polymorph: {
    id: 'true_polymorph',
    name: 'True Polymorph',
    level: 9,
    ...
    rangeKind: 'ranged',
    rangeFt: 60,
    desc: 'A creature within 60 ft makes a WIS save or is transformed into a beast (Wolf, 11 HP) for the duration. Concentration, up to 1 hour (RAW becomes permanent).',
    narrative: '{name} speaks a word of true change — {target} is unmade and remade as a beast.',
    spellList: ['arcane'],
  },

  // ─── Catalog completion: effects pansori has no faithful combat model for ─────
  // The SRD's remaining spells turn on systems this engine doesn't have — magic
  // suppression, creature-type interactions, possession, self/mass shapeshifting
  // into arbitrary creatures, stored/triggered spells, extra turns, planar
  // gating, and reality alteration. They're registered as narrative spells (their
  // effect is described and adjudicated at the table) so the catalog is complete;
  // full mechanization waits on those systems.
  enthrall: {
    id: 'enthrall',
    name: 'Enthrall',
    level: 2,
    ...
    rangeKind: 'ranged',
    rangeFt: 60,
    desc: 'Weave a distracting string of words; affected creatures have Disadvantage on Perception checks to notice anyone but you for 1 minute (WIS save resists).',
    narrative: '{name} spins a mesmerizing patter; nearby foes struggle to look away.',
    spellList: ['arcane'],
  },
  creation: {
    id: 'creation',
    ...
    rangeKind: 'ranged',
    rangeFt: 30,
    desc: 'Pull a mass of vegetable or mineral matter into a nonliving object of your design — its size and material set how long it lasts.',
    narrative: '{name} draws shadowstuff into being as a solid, if temporary, object.',
    spellList: ['arcane'],
  },
  dispel_evil_and_good: {
    ...
    concentration: true,
    rangeKind: 'self',
    desc: 'Celestials, elementals, fey, fiends, and undead have Disadvantage on attacks against you; you can also break enchantments or hurl such a creature away. Concentration.',
    narrative: '{name} is wreathed in warding power that turns aside otherworldly foes.',
    spellList: ['divine'],
  },
  antilife_shell: {
    id: 'antilife_shell',
    ...
    concentration: true,
    rangeKind: 'self',
    desc: 'A shimmering barrier, 10 ft around you, holds living creatures at bay — they can’t pass through or reach across it. Concentration, up to 1 hour.',
    narrative: '{name} raises a barrier that living things simply cannot push through.',
    spellList: ['primal'],
  },
  globe_of_invulnerability: {
    ...
    concentration: true,
    rangeKind: 'self',
    magicSuppression: { maxLevel: 5, fromOutsideOnly: true },
    desc: 'A shimmering 10-ft globe surrounds you; spells of 5th level and lower cast from outside cannot affect anything within it. Concentration.',
    narrative: '{name} is enclosed in a faint, humming sphere that swallows lesser magic.',
    spellList: ['arcane'],
  },
  contingency: {
    id: 'contingency',
    ...
    outOfCombatOnly: true,
    rangeKind: 'self',
    desc: 'Store a spell (5th level or lower that targets you) with a trigger condition; it casts automatically the instant that condition occurs, once within 10 days.',
    narrative: '{name} folds a second spell into readiness, primed to spring when needed.',
    spellList: ['arcane'],
  },
  magic_jar: {
    id: 'magic_jar',
    ...
    outOfCombatOnly: true,
    rangeKind: 'self',
    desc: 'Project your soul into a prepared vessel, then attempt to seize the body of a nearby creature (CHA save resists), leaving your own body empty.',
    narrative: "{name}'s soul slips free into a waiting gem, reaching to wear another's body.",
    spellList: ['arcane'],
  },
  planar_ally: {
    id: 'planar_ally',
    ...
    outOfCombatOnly: true,
    rangeKind: 'self',
    desc: 'Beseech your deity to send a celestial, elemental, or fiend to aid you — the creature appears and bargains over the service and its price.',
    narrative: '{name} calls across the planes; a servant of their faith answers, for a price.',
    spellList: ['divine'],
  },
  planar_binding: {
    id: 'planar_binding',
    ...
    rangeKind: 'ranged',
    rangeFt: 60,
    desc: 'Bind a celestial, elemental, fey, or fiend (already summoned or present) into your service for up to 24 hours, if it fails a CHA save.',
    narrative: '{name} chains an otherworldly creature to their will with a binding rite.',
    spellList: ['arcane', 'divine', 'primal'],
  },
  project_image: {
    id: 'project_image',
    ...
    outOfCombatOnly: true,
    rangeKind: 'self',
    desc: 'Create an illusory double of yourself anywhere on the same plane you’ve seen; you can see and speak through it as if present. Concentration.',
    narrative: '{name} casts a perfect double of themselves to walk and speak far away.',
    spellList: ['arcane'],
  },
  simulacrum: {
    id: 'simulacrum',
    ...
    outOfCombatOnly: true,
    rangeKind: 'touch',
    desc: 'Over 12 hours, shape snow and ice into a duplicate of a creature with half its hit points and no ability to recover — an obedient simulacrum.',
    narrative: '{name} sculpts an icy twin that rises, obedient and hollow-eyed.',
    spellList: ['arcane'],
  },
  animal_shapes: {
    id: 'animal_shapes',
    ...
    castTime: 'action',
    concentration: true,
    shapeshift: { scope: 'allies' },
    rangeKind: 'ranged',
    rangeFt: 30,
    desc: "Transform the willing party into beasts (one chosen form) — each gains the form's attacks + temporary HP — for the duration. Concentration, up to 1 day. (Beasts cap at CR 1.)",
    narrative: '{name} sweeps a hand and willing allies take on the shapes of beasts.',
    spellList: ['primal'],
  },
  antimagic_field: {
    id: 'antimagic_field',
    ...
    concentration: true,
    rangeKind: 'self',
    magicSuppression: { fromOutsideOnly: false },
    desc: 'A 10-ft sphere of dead magic moves with you: spells fail for anyone casting into or out of it. Concentration. (The magic-item / ongoing-effect suppression is narrative.)',
    narrative: '{name} is surrounded by a bubble where magic simply stops working.',
    spellList: ['divine', 'arcane'],
  },
  foresight: {
    id: 'foresight',
    ...
    outOfCombatOnly: true,
    rangeKind: 'touch',
    desc: "Grant a creature uncanny prescience for 8 hours: it can't be surprised, has Advantage on attacks, saves, and ability checks, and attackers have Disadvantage against it.",
    narrative: '{name} grants {target} a sliver of the future — every danger felt before it comes.',
    spellList: ['arcane', 'primal'],
  },
  gate: {
    id: 'gate',
    ...
    rangeKind: 'ranged',
    rangeFt: 60,
    desc: 'Tear open a shimmering portal linking to a chosen plane; you can travel through it or call a specific creature whose name you speak. Concentration.',
    narrative: '{name} rips a ringed portal into the air, opening onto another world.',
    spellList: ['divine', 'arcane'],
  },
  shapechange: {
    id: 'shapechange',
    ...
    castTime: 'action',
    concentration: true,
    shapeshift: { scope: 'self' },
    rangeKind: 'self',
    desc: 'Assume a beast form (CR ≤ your level), gaining its attacks + temporary HP while keeping your mind, for the duration. Concentration, up to 1 hour. (Beasts cap at CR 1.)',
    narrative: '{name} flows into the shape of a mightier creature, mind intact.',
    spellList: ['primal', 'arcane'],
  },
  time_stop: {
    id: 'time_stop',
    ...
    castTime: 'action',
    rangeKind: 'self',
    grantsExtraTurns: '1d4+1',
    desc: 'Stop time for everyone but you, taking 1d4 + 1 turns in a row — the effect ends early the moment one of those turns affects an enemy.',
    narrative: '{name} steps outside the flow of time; for a few heartbeats, only they move.',
    spellList: ['arcane'],
  },
  wish: {
    id: 'wish',
    ...
    castTime: 'action',
    rangeKind: 'self',
    desc: 'The mightiest spell: duplicate any spell of 8th level or lower, or bend reality to another effect of comparable power — the greater the wish, the greater the risk.',
  ...
  true_strike: {
    ...
    castTime: 'action',
    rangeKind: 'self',
    desc: 'Make one attack with the weapon used to cast this spell, using your spellcasting ability for the roll; on a hit it deals an extra 1d6 Radiant damage (more at higher levels).',
    narrative: '{name} channels the spell down the blade and strikes with guided precision.',
    spellList: ['arcane'],
  },
};
