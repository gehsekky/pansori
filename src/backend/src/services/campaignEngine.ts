...
export async function evaluateQuestSteps(
  ...
  for (const qp of active) {
    ...
    for (const step of def.steps) {
      ...
      engine.addRule({ name: step.id, conditions: step.condition as TopLevelCondition, event: { type: 'step_met' } });
    ...
    if (newlyCompleted.length) results.push({ questId: qp.questId, completedStepIds: newlyCompleted });
  }
...
export function applyQuestCompletions(
  ...
  let updated = { ...cs, quests: cs.quests.map(q => ({ ...q, completedSteps: [...q.completedSteps] })) };
...
export function getNpcAttitude(cs: CampaignState, npcId: string, defaultAttitude: NpcAttitude = 'indifferent'): NpcAttitude {
