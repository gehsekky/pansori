...
describe('Sorcery Incarnate (L7) — activate Innate Sorcery via 2 SP', () => {
  it('spends 2 sorcery points to activate when out of free uses', () => {
    const c = featCtx(makeChar({ character_class: 'Sorcerer', level: 7, cha: 16, turn_actions: fresh(), class_resource_uses: { innate_sorcery_used: 2, sorcery_points: 5 } }));
  ...
  it('a L6 sorcerer out of uses cannot (no Sorcery Incarnate)', () => {
    const c = featCtx(makeChar({ character_class: 'Sorcerer', level: 6, turn_actions: fresh(), class_resource_uses: { innate_sorcery_used: 2, sorcery_points: 5 } }));
    handleCasterFeature(c, 'innate_sorcery');
...
describe('Sorcery Incarnate (L7) — two Metamagics stack while Innate active', () => {
  it('a second metamagic stacks (Innate active, L7)', () => {
    const c = featCtx(makeChar({ character_class: 'Sorcerer', level: 7, cha: 16, turn_actions: fresh(), conditions: ['innate_sorcery'], metamagics_known: ['empowered', 'distant'], class_resource_uses: { sorcery_points: 9 } }));
  ...
  it('without Innate Sorcery active, the second metamagic replaces the first', () => {
    const c = featCtx(makeChar({ character_class: 'Sorcerer', level: 7, cha: 16, turn_actions: fresh(), metamagics_known: ['empowered', 'distant'], class_resource_uses: { sorcery_points: 9 } }));
    handleCasterFeature(c, 'metamagic_empowered');
    handleCasterFeature(c, 'metamagic_distant');
...
describe('Arcane Apotheosis (L20) — one free Metamagic per turn while Innate active', () => {
  it('the first metamagic is free; the second costs sorcery points', () => {
    const c = featCtx(makeChar({ character_class: 'Sorcerer', level: 20, cha: 16, turn_actions: fresh(), conditions: ['innate_sorcery'], metamagics_known: ['empowered', 'distant'], class_resource_uses: { sorcery_points: 6 } }));
  ...
  it('a L19 sorcerer pays for every metamagic (no Apotheosis)', () => {
    const c = featCtx(makeChar({ character_class: 'Sorcerer', level: 19, cha: 16, turn_actions: fresh(), conditions: ['innate_sorcery'], metamagics_known: ['empowered'], class_resource_uses: { sorcery_points: 6 } }));
    handleCasterFeature(c, 'metamagic_empowered');
