...
describe('Defensive Tactics helpers', () => {
  it('hasEscapeTheHorde gates on Hunter + L7 + the choice', () => {
    ...
    expect(hasEscapeTheHorde(hunter({ level: 6, defensive_tactics: 'escape_the_horde' }))).toBe(false);
    expect(hasEscapeTheHorde(makeChar({ character_class: 'Ranger', level: 7, defensive_tactics: 'escape_the_horde' }))).toBe(false); // no subclass
  });
...
describe('choose_hunter_option — Defensive Tactics', () => {
  it('a Hunter L7 can choose Escape the Horde or Multiattack Defense', () => {
    ...
    handleChooseHunterOption(c1, { type: 'choose_hunter_option', feature: 'defensive_tactics', option: 'escape_the_horde' });
    expect(pcChar(c1).defensive_tactics).toBe('escape_the_horde');

    const c2 = featCtx(hunter());
    handleChooseHunterOption(c2, { type: 'choose_hunter_option', feature: 'defensive_tactics', option: 'multiattack_defense' });
    expect(pcChar(c2).defensive_tactics).toBe('multiattack_defense');
  });

  it('requires level 7', () => {
    const c = featCtx(hunter({ level: 6 }));
    handleChooseHunterOption(c, { type: 'choose_hunter_option', feature: 'defensive_tactics', option: 'escape_the_horde' });
  ...
  it('surfaces both Defensive Tactics options to a Hunter L7 out of combat (not at L6)', () => {
    const offered = (level: number) => {
      ...
      return generateChoices(state, baseSeed, ctx)
        .filter((c) => c.action.type === 'choose_hunter_option' && c.action.feature === 'defensive_tactics')
...
describe('Multiattack Defense — integration (two enemy swings)', () => {
  it("the attacker's second swing rolls with Disadvantage after the first hits", () => {
    ...
    vi.spyOn(Math, 'random').mockReturnValueOnce(0.9).mockReturnValueOnce(0.9).mockReturnValueOnce(0.05);
    const target = hunter({ id: 'pc-1', hp: 20, max_hp: 20, ac: 15, defensive_tactics: 'multiattack_defense' });
    const c1 = enemyCtx(target);
    handleEnemyAttack(c1, { type: 'enemy_attack', targetCharId: 'pc-1', advIdx: 0, multiattackIdx: 0 });
    ...
    handleEnemyAttack(c2, { type: 'enemy_attack', targetCharId: 'pc-1', advIdx: 0, multiattackIdx: 1 });
  ...
  it('a non-Multiattack-Defense PC takes the second hit (control)', () => {
    ...
    const target = hunter({ id: 'pc-1', hp: 20, max_hp: 20, ac: 15, defensive_tactics: 'escape_the_horde' });
    const c1 = enemyCtx(target);
    handleEnemyAttack(c1, { type: 'enemy_attack', targetCharId: 'pc-1', advIdx: 0, multiattackIdx: 0 });
    const t1 = c1.enemySubAttack?.outcome === 'done' ? c1.enemySubAttack.target : target;
    const c2 = enemyCtx(t1);
