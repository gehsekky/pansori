...
export function isFlankingPosition(
  attacker: GridPos,
  ally: GridPos,
  target: GridPos,
): boolean {
  ...
  return (Math.sign(ax) === -Math.sign(bx) && bx !== 0) ||
         (Math.sign(ay) === -Math.sign(by) && by !== 0);
}
...
export function opportunityAttackTriggers(
  ...
  return entities.filter(
    e => e.isEnemy !== moverIsEnemy && wasAdjacent(e) && !isAdjacent(e),
  );
}
