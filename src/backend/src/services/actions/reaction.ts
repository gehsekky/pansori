...
export const handleResolveReaction: ActionHandler<{
  ...
  } else if (rx.kind === 'restore_balance') {
    // 2024 PHB Clockwork Soul Sorcerer L3 — Restore Balance. Accept
    // spends 1 use + the reactor's reaction, then re-rolls the enemy
    // d20 flat (no advantage). If the flat d20 falls below AC, the
    // attack becomes a miss (damage discarded). Otherwise the hit
    // stands. Decline = commit the full-damage proposed snapshot.
    const pendingFragment = rx.pendingFragment as EnemyAttackHitFragment;
    const pendingProposedChar = rx.pendingProposedChar as Character;
    const pendingProposedSt = rx.pendingProposedSt as GameState;
    const targetCharId = rx.targetCharId;
    if (action.accept) {
      const remaining = ctx.char.class_resource_uses?.restore_balance_uses ?? 0;
      if (remaining <= 0) {
        ctx.narrative = `No Restore Balance uses available. ${pendingFragment.prose}`;
        ctx.st = {
          ...ctx.st,
          characters: pendingProposedSt.characters.map((c) =>
            c.id === targetCharId ? pendingProposedChar : c
          ),
          entities: (pendingProposedSt.entities ?? ctx.st.entities ?? []).map((e) =>
            e.id === targetCharId && !e.isEnemy ? { ...e, hp: pendingProposedChar.hp } : e
          ),
          pending_reaction: undefined,
        };
        ctx.st = pushEvent(ctx.st, enemyAttackFragmentEvent(pendingFragment, ctx.st.round ?? 1));
      } else {
        // Spend 1 use + reaction. Re-roll the enemy d20 flat — no
        // adv/disadv, single d20.
        const newD20 = rollDice('1d20');
        const mods = rx.atkTotal - rx.proposedD20;
        const newTotal = newD20 + mods;
        const newHit = newTotal >= rx.targetAc;
        // The reactor (ctx.char) consumes the resource + reaction.
        const reactorAfter: Character = {
          ...ctx.char,
          class_resource_uses: {
            ...(ctx.char.class_resource_uses ?? {}),
            restore_balance_uses: remaining - 1,
          },
          turn_actions: { ...ctx.char.turn_actions, reaction_used: true },
        };
        if (newHit) {
          // Reroll still hits — commit proposed (with damage) but
          // keep the reactor's resource + reaction spend. The
          // target is whoever was hit (may be a different PC than
          // the reactor).
          ctx.st = {
            ...ctx.st,
            characters: pendingProposedSt.characters.map((c) => {
              if (c.id === targetCharId) return pendingProposedChar;
              if (c.id === ctx.char.id) return reactorAfter;
              return c;
            }),
            entities: (pendingProposedSt.entities ?? ctx.st.entities ?? []).map((e) =>
              e.id === targetCharId && !e.isEnemy ? { ...e, hp: pendingProposedChar.hp } : e
          ...
          ctx.char = reactorAfter;
          ctx.st = pushEvent(ctx.st, enemyAttackFragmentEvent(pendingFragment, ctx.st.round ?? 1));
          ctx.narrative = `⚖️ ${ctx.char.name} channels Restore Balance — flat reroll d20 ${newD20}, total ${newTotal} vs AC ${rx.targetAc} — still hits. ${pendingFragment.prose}`;
        } else {
          // Reroll turns the hit into a miss — damage discarded.
          // Reactor's resource + reaction still consumed.
          ctx.st = {
            ...ctx.st,
            characters: ctx.st.characters.map((c) => (c.id === ctx.char.id ? reactorAfter : c)),
            pending_reaction: undefined,
          };
          ctx.char = reactorAfter;
          ctx.st = pushEvent(ctx.st, {
          ...
          ctx.narrative = `⚖️ ${ctx.char.name} channels Restore Balance — flat reroll d20 ${newD20}, total ${newTotal} vs AC ${rx.targetAc} — the strike misses!`;
        }
      }
    } else {
      // Decline — commit full proposed snapshot. Resource retained.
      ctx.st = {
        ...ctx.st,
        characters: pendingProposedSt.characters.map((c) =>
          c.id === targetCharId ? pendingProposedChar : c
      ...
      ctx.st = pushEvent(ctx.st, enemyAttackFragmentEvent(pendingFragment, ctx.st.round ?? 1));
      ctx.narrative = `${pendingFragment.prose} (Restore Balance declined.)`;
    }
