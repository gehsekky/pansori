...
import type { CampaignData, GameState, Room } from '../../src/types.js';
import {
  ENCOUNTER_ROOM_ID,
  activeGrid,
  initMapState,
  ...
  returnFromEncounter,
  stageEncounter,
} from '../../src/services/mapEngine.js';
...
describe('SRD travel pace (set_pace + perception effects)', () => {
  it('set_pace stores the stance and narrates it', async () => {
    const { takeAction } = await import('../../src/services/gameEngine.js');
    const { makeState } = await import('../../src/test-fixtures.js');
    const { context: sandbox } = await import('../../src/campaignData/sandbox.js');
    const st = {
      ...makeState({ id: 'pc-1' }, {}),
  ...
  it('pacePerceptionMod: fast −5 / slow +5 swings passive trap detection', async () => {
    const { pacePerceptionMod, partyDetectsTrap } =
      await import('../../src/services/gameEngine.js');
    const chars = [
