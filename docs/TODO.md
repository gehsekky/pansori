...
- [~] **Playwright E2E test** — two tests in `tests/e2e/vale-smoke.spec.ts`: (a) Vale smoke test covers login → sessions → char screen → world pick → auto-fill party → BEGIN MISSION → narrative renders; (b) sandbox combat test drives the choice loop until an Attack action surfaces, asserting initiative + grid_move + attack pipeline work end-to-end. `data-testid` attrs on: new-mission-btn, world-picker-{id}, auto-fill-party-btn, begin-mission-btn, game-narrative-panel, choice-btn (+ `data-action-type`). Auth bypass at `/api/auth/test-login` double-gated (`NODE_ENV!=production` AND `E2E_TEST_LOGIN_ENABLED=true`); dev compose sets the flag, prod compose doesn't. Combat test has `retries: 2` for procgen variance (~80% pass first try, 100% with retries across 5 runs). Run with `npm run test:e2e` (needs `npm run test:e2e:install` once + system libs `libasound2 libnspr4 libnss3` or the Playwright docker image — see `tests/e2e/README.md`). Still TODO: (a) wire into CI as a separate workflow, (b) expand coverage to session resume + faction price modifiers + quest completion (the top-5 #1 acceptance criteria still needs these to close).
