...
2. **Combat narrative clarity (UX) — iterate** — foundation (CombatEvent + CombatLogPanel) shipped; remaining work is event coverage edge cases (Abjure Enemy frighten, Fey Presence frighten, Goading Attack, etc.) and visual polish (event grouping, color theming).
