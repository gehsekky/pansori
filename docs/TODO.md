...
- [~] **Playwright E2E smoke test** — infrastructure landed. One test in `tests/e2e/vale-smoke.spec.ts` covers login → sessions → char screen → world pick → auto-fill party → BEGIN MISSION → narrative renders. `data-testid` attrs added to: new-mission-btn, world-picker-{id}, auto-fill-party-btn, begin-mission-btn, game-narrative-panel. Auth bypass at `/api/auth/test-login` double-gated (`NODE_ENV!=production` AND `E2E_TEST_LOGIN_ENABLED=true`); dev compose sets the flag, prod compose doesn't. Run with `npm run test:e2e` (needs `npm run test:e2e:install` once + system libs `libasound2 libnspr4 libnss3` or the Playwright docker image — see `tests/e2e/README.md`). Still TODO: (a) wire into CI as a separate workflow, (b) expand coverage to combat resolution + session resume + faction price modifiers (the top-5 #1 acceptance criteria), but only after the smoke test proves stable.
