...
- [ ] **Narrative speaker clarity** — in a multi-PC party the combat log mixes turns from each character but most narratives start with "You ..." instead of the character's name. The reader can't tell whether "You strike the bandit for 5 damage" was Aurelia, Brennan, or Wren. Fix in two passes: (1) audit the ~34 hard-coded `"You ..."` narratives in `gameEngine.ts` and replace `You` with `${char.name}`; (2) longer-term, route every narrative line through a `narrate(template, ctx)` helper that prepends `[CharName]` consistently for multi-PC parties. The same helper can substitute `{level}`, `{name}`, `{enemy}` etc. so the level-up `{level}`-not-substituted bug (fixed in this same commit) can't recur silently.
- [ ] **Placeholder lint** — small test that scans every string in `context.narratives` for unbracketed substitution tokens (`{X}`) and confirms the consuming code in `gameEngine.ts` has a corresponding `.replace(...)` for that token. Would have caught the level-up bug at CI time.
