...
- [~] **Magic items** — content; the body-slot + attunement + curse + worn-effect
      ...
      **Ring of Protection** (a new `ac_bonus` worn-effect kind folded into the
      ...
      five Belts of Giant Strength (STR 21–29). **+N magic weapons & armor** also
      shipped — a `LootItem.magicBonus` field: on a weapon, +N to attack + damage
      and the attack counts as **magical** (bypasses nonmagical resistance); on
      armor / shields, +N to AC (folded into `computeTotalAc`). The bestiary now
      distinguishes `nonmagical_resistances` (10 monsters — elementals, undead,
      golems, Tarrasque) from true B/P/S resistance (swarms, treants). Items are
      generated by cloning a base item (no stat drift): longsword +1/+2/+3,
      dagger/greataxe/longbow/mace +1, studded-leather/half-plate/plate +N,
